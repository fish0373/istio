...
func TestSdsCertificateConfigFromResourceName(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			false,
			true,
			SdsCertificateConfig{CertificatePath: "cert", PrivateKeyPath: "key"},
			"",
			"file-cert:cert~key",
		},
		{
			...
			true,
			false,
			SdsCertificateConfig{CaCertificatePath: "root"},
			"file-root:root",
			"",
		},
		{
			...
			true,
			false,
			SdsCertificateConfig{CaCertificatePath: "root", CRLPath: "crl"},
			"file-root:root~crl",
			"",
		},
		{
